...
const programAddress = new PublicKey(
  "9TYPgvadsErCiq1PiZ3Us9fY52eLFVhTHnZ9gZUNiEVT"
);
...
describe("contracts", () => {
  ...
  beforeEach(async () => {
    context = await startAnchor(
      "",
      [{ name: "contracts", programId: programAddress }],
      []
    );
  ...
  it("Create a delivery", async () => {
    ...
    const [deliveryAddress] = PublicKey.findProgramAddressSync(
      [
  ...
  it("Accept Delivery Job", async () => {
    ...
    await program.methods
      .acceptDeliveryJob(index, publicAddress)
      .accountsPartial({ courier: courier.publicKey })
      .signers([courier])
      .rpc();
    let [deliveryAddress] = PublicKey.findProgramAddressSync(
      [publicAddress.toBuffer(), index.toArrayLike(Buffer, "le", 8)],
      programAddress
    );
    let delivery = await program.account.delivery.fetch(deliveryAddress);
  ...
  it("Confirm a Delivery", async () => {
    ...
    await program.methods
      ...
      .accountsPartial({ courierAccount: courier.publicKey })
      .rpc();
